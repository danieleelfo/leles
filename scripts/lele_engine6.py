...
# 💬 MAIN LOOP
def main():
    print("🏴‍☠️ Lele Engine FIXED v6 (stable)\n")
    ...
    while True:
        ...
        # STEP 1: GEMMA
        trigger_llama = any(word in user_input.lower() for word in ["edita", "review", "roast", "llama", "critica"])

        if trigger_llama:
            gemma_out = "[SKIPPED - LLAMA MODE]"
        ...
        if gemma_out != "[SKIPPED - LLAMA MODE]":save_memory("GEMMA", gemma_out)
